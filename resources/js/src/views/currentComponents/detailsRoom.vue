...
<div>
    ...
    <b-card class="pp-custom"  bg-variant="light">
    <div class="d-flex flex-wrap co-12 mt-1">
        <p class="col col-12 font-weight-bolder text-black mmb-2">Nombre: <span class="font-weight-normal">{{ habitacion.nombre }}</span></p>
        <p class="col col-12 font-weight-bolder text-black mmb-2">Capacidad: <span class="font-weight-normal">{{ habitacion.capacidad }} {{ habitacion.capacidad == 1 ? 'persona' :' personas' }}</span></p>
        <!-- <p class="col col-12 font-weight-bolder text-black mmb-2">Tarifa: <span class="font-weight-normal">{{ this.formatoMoney(habitacion.tarifa) }}</span></p> -->
        <p class="col col-12 font-weight-bolder text-black mmb-2">Tarifa: <span class="font-weight-normal">{{ (habitacion.tarifa) }}</span></p>
        <p class="col col-12 font-weight-bolder text-black mmb-2">¿Se puede fumar?: <span class="font-weight-normal">{{ habitacion.puedeFumar?'Si':'No' }} se puede fumar</span></p>
    </div>
    <div class="d-flex justify-content-between pt-1 pb-1">
        <b-button v-if="showButton" variant="relief-primary" @click="handleSeleccionar">
            Seleccionar
        </b-button>
        <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="secondary" class="btn-icon rounded-circle" @click="handleModal">
